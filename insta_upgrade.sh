...
.i-cap .i-name{margin-left:5px}
</style>
<script>
// ========== REAL INSTAGRAM UPGRADE ==========
...
if 'HEART_F' not in content:
    content = content.replace('</body>', upgrade + '</body>', 1)
    print('✅ Instagram real-style upgrade applied!')
else:
...
git add -A && git commit -m "Real Instagram style: outline icons + Arabic numbers + relative time" && git push
